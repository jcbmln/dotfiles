    ...
    function git_commits_ahead() {
        ...
        if [[ "$commits" != 0 ]]; then
            echo "$GIT_COMMITS_AHEAD_PREFIX$commits$GIT_COMMITS_AHEAD_SUFFIX"
        fi
        fi
    }
    ...
    function git_commits_behind() {
        ...
        if [[ "$commits" != 0 ]]; then
            echo "$GIT_COMMITS_BEHIND_PREFIX$commits$GIT_COMMITS_BEHIND_SUFFIX"
        fi
        fi
    }
    ...
    function git_prompt_remote() {
        ...
        echo "$GIT_PROMPT_REMOTE_EXISTS"
        else
        echo "$GIT_PROMPT_REMOTE_MISSING"
        fi
    }
    ...
    function git_prompt_long_sha() {
        local SHA
        SHA=$(command git rev-parse HEAD 2> /dev/null) && echo "$GIT_PROMPT_SHA_BEFORE$SHA$GIT_PROMPT_SHA_AFTER"
    }
